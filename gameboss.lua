...
--爆炸群組
local explosionGroup2 = display.newGroup()
...
function scene:create( event )
    ...
    --預先載入音效
    sounds = {
	music2 = audio.loadSound("coronaboss.mp3"),
	laser2 = audio.loadSound("laserFire.wav"),
	explosion2 = audio.loadSound("explosion.wav"),
   }
    ...
    --背景
    local bg = display.newImageRect( "bgn3.jpg", 480, 320 )
    local bg1 = display.newImageRect( "bgn4.jpg", 480, 320)
       bg.anchorX = 0
       bg.x = 0
       bg.y = centerY
       bg.speed = 1
...
local function asdd(asdd)
	print( "asd called" )
	createEnemy() 
	
end
timer.performWithDelay(1000,asdd,0)
local function changeScene()
  print("changeScene")
   timer.cancel(mytime)
   composer.gotoScene("game_boss") 
...
local function explode( x,y )
	...
	explosion.y=y
	explosion:play()
	explosionGroup2:insert(explosion)
	--print("explosionGroup numChildren".. explosionGroup.numChildren)
	audio.play( sounds.explosion2 )
math.random (-500, -100)
end
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        audio.play( sounds.music2, { channel=3, loops=-1})
